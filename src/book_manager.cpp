...
bool BookManager::processBookSettings(sqlite3* db, int bookId, const BookMetadata& metadata, int profileId) {
    ...
    // Парсинг даты завершения чтения
    ...
    if (exists) {
        ...
        if (metadata.isRead) {
            // СЦЕНАРИЙ 1: Книга помечена как ПРОЧИТАННАЯ.
            // Мы принудительно выставляем cpage=100, npage=100.
            // Это соответствует логике "сброса в 100%" из lua скрипта.
            ...
            if (sqlite3_prepare_v2(db, updateSql, -1, &stmt, nullptr) == SQLITE_OK) {
                sqlite3_bind_int(stmt, 1, completed); // 1
                sqlite3_bind_int(stmt, 2, favorite);
                sqlite3_bind_int64(stmt, 3, completedTs);
                sqlite3_bind_int(stmt, 4, 100); // cpage
                sqlite3_bind_int(stmt, 5, 100); // npage
            ...
            // СЦЕНАРИЙ 2: Книга НЕ прочитана (но, возможно, изменено "Избранное").
            // Мы НЕ меняем cpage и npage. Мы обновляем только статус и флаги.
            // Это отличается от pb-db.lua (который сбрасывает в 0), но это то, 
            // что нужно пользователю для сохранения прогресса.
            const char* updateSql = 
                "UPDATE books_settings "
            ...
            if (sqlite3_prepare_v2(db, updateSql, -1, &stmt, nullptr) == SQLITE_OK) {
                sqlite3_bind_int(stmt, 1, completed); // 0
                sqlite3_bind_int(stmt, 2, favorite);
                sqlite3_bind_int64(stmt, 3, completedTs);
        ...
        // Здесь у нас нет предыдущего прогресса, поэтому ставим значения по умолчанию.
